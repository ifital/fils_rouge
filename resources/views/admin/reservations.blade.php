...
<head>
    ...
    <title>Blue Waves - Reservations</title>
        ...
        >
            ...
            <nav class="py-4">
                <ul class="space-y-1">
                    ...
                    <li>
                        <a href="{{ Route('admin.reservations') }}" class="flex items-center px-4 py-3 text-yellow-400 bg-gray-900 border-l-4 border-yellow-400">
                            <span class="sidebar-icon mr-3">
        ...
        <main class="flex-1 p-4 md:p-6 w-full overflow-x-hidden">
            <div class="max-w-7xl mx-auto">
                ...
                <div class="mb-6 md:mb-8">
                    <h1 class="text-xl md:text-2xl font-bold text-black">Reservations</h1>
                    <p class="text-gray-600">Manage all guest reservations</p>
                </div>
                <!-- Reservations Table -->
                        ...
                        <table class="min-w-full">
                            <thead class="bg-gray-100 text-gray-700">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-sm font-medium">Guests</th>
                            ...
                            <tbody>
                                @foreach($reservations as $reservation)
                                    <tr class="border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 text-sm">{{ $reservation->room->room_number }} ({{ ucfirst($reservation->room->type) }})</td>
                                        ...
                                        <td class="px-4 py-3 text-sm text-center">{{ $reservation->guests ?? '-' }}</td>
                                        <td class="px-4 py-3 text-sm">{{ number_format($reservation->price, 2) }} MAD</td>
                                        <td class="px-4 py-3 text-sm">
                                            <span class="px-2 py-1 rounded-full text-xs
                                                ...
                                                ($reservation->status == 'completed' ? 'bg-blue-200 text-blue-800' : 'bg-red-200 text-red-800')) }}">
                                        ...
                                        <td class="px-4 py-3 text-sm">
                                            @if($reservation->payment)
                                                <span class="px-2 py-1 rounded-full text-xs
                                                    {{ $reservation->payment->status == 'paid' ? 'bg-green-200 text-green-800' : 'bg-yellow-200 text-yellow-800' }}">
                                                    {{ ucfirst($reservation->payment->status) }}
                                                </span>
                                            @else
                                                <span class="px-2 py-1 rounded-full text-xs bg-gray-200 text-gray-800">Unpaid</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
